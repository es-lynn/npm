import { Misc, NotImplementedErr } from '@es-lynn/utils'
...
export class AwsDynamodb<T extends { id: string }> implements NoSQLDatabase<T> {
    ...
    return new Promise((resolve, reject) => {
      this.ddb.get(params, (err: any, data: any) => {
        if (err) {
          ...
          reject(
            new Error(`No items retrieved from: ${this.table} : {id:${id}}`)
          )
        } else {
  ...
  async deleteAll(): Promise<void> {
    throw new NotImplementedErr(
      'AwsDynamodb.deleteAll() has not yet been implemented'
    )
  }
}
